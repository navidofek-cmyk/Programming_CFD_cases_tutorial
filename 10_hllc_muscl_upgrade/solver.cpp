...
PrimitiveState reconstruct_x_right(const Mesh2D& mesh,
                                   const PrimitiveFields& primitive,
                                   int i,
                                   int j) {
    if (i <= 0 || i >= mesh.nx - 1 || mesh.is_solid(i - 1, j) || mesh.is_solid(i, j) || mesh.is_solid(i + 1, j)) {
        return primitive_at(primitive, i, j);
    }

    const PrimitiveState qm = primitive_at(primitive, i - 1, j);
    const PrimitiveState q0 = primitive_at(primitive, i, j);
    const PrimitiveState qp = primitive_at(primitive, i + 1, j);

    return {
        q0.rho + 0.5 * minmod(q0.rho - qm.rho, qp.rho - q0.rho),
        q0.u   + 0.5 * minmod(q0.u   - qm.u,   qp.u   - q0.u),
        q0.v   + 0.5 * minmod(q0.v   - qm.v,   qp.v   - q0.v),
        q0.p   + 0.5 * minmod(q0.p   - qm.p,   qp.p   - q0.p)
    };
}

PrimitiveState reconstruct_x_left(const Mesh2D& mesh,
                                  const PrimitiveFields& primitive,
                                  int i,
                                  int j) {
    if (i <= 0 || i >= mesh.nx - 1 || mesh.is_solid(i - 1, j) || mesh.is_solid(i, j) || mesh.is_solid(i + 1, j)) {
...
PrimitiveState reconstruct_y_top(const Mesh2D& mesh,
                                 const PrimitiveFields& primitive,
                                 int i,
                                 int j) {
    if (j <= 0 || j >= mesh.ny - 1 || mesh.is_solid(i, j - 1) || mesh.is_solid(i, j) || mesh.is_solid(i, j + 1)) {
        return primitive_at(primitive, i, j);
    }

    const PrimitiveState qm = primitive_at(primitive, i, j - 1);
    const PrimitiveState q0 = primitive_at(primitive, i, j);
    const PrimitiveState qp = primitive_at(primitive, i, j + 1);
...
PrimitiveState reconstruct_y_bottom(const Mesh2D& mesh,
                                    const PrimitiveFields& primitive,
                                    int i,
                                    int j) {
    if (j <= 0 || j >= mesh.ny - 1 || mesh.is_solid(i, j - 1) || mesh.is_solid(i, j) || mesh.is_solid(i, j + 1)) {
...
StepReport advance_euler_step(const Mesh2D& mesh,
    ...
    for (int j = 0; j < mesh.ny; ++j) {
        for (int i = 0; i < mesh.nx; ++i) {
            ...
            const PrimitiveState center_east =
                reconstruct_x_right(mesh, primitive, i, j);
            const PrimitiveState center_west =
                reconstruct_x_left(mesh, primitive, i, j);
            ...
            const PrimitiveState center_north =
                reconstruct_y_top(mesh, primitive, i, j);
            const PrimitiveState center_south =
                reconstruct_y_bottom(mesh, primitive, i, j);
            const PrimitiveState top_face =
                face_top_state(mesh, primitive, i, j + 1);

            const PrimitiveState east_left = center_east;
            PrimitiveState east_right = right_face;
            if (i + 1 < mesh.nx && mesh.is_fluid(i + 1, j)) {
                east_right = reconstruct_x_left(mesh, primitive, i + 1, j);
            }

            PrimitiveState west_left = left_face;
            const PrimitiveState west_right = center_west;
            if (i - 1 >= 0 && mesh.is_fluid(i - 1, j)) {
                west_left = reconstruct_x_right(mesh, primitive, i - 1, j);
            }

            const PrimitiveState north_bottom = center_north;
            PrimitiveState north_top = top_face;
            if (j + 1 < mesh.ny && mesh.is_fluid(i, j + 1)) {
                north_top = reconstruct_y_bottom(mesh, primitive, i, j + 1);
            }

            PrimitiveState south_bottom = bottom_face;
            const PrimitiveState south_top = center_south;
            if (j - 1 >= 0 && mesh.is_fluid(i, j - 1)) {
                south_bottom = reconstruct_y_top(mesh, primitive, i, j - 1);
            }
            ...
            hllc_flux_x(east_left, east_right, gamma_value, Fe1, Fe2, Fe3, Fe4);
            hllc_flux_x(west_left, west_right, gamma_value, Fw1, Fw2, Fw3, Fw4);
            hllc_flux_y(north_bottom, north_top, gamma_value, Gn1, Gn2, Gn3, Gn4);
            hllc_flux_y(south_bottom, south_top, gamma_value, Gs1, Gs2, Gs3, Gs4);
